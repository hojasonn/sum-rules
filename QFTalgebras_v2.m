...
vec::usage = "vec[p,\[Mu]] denotes a four vector with Lorentz index \[Mu] and symbol p. No distinction between position and momentum vectors."
...
integralDef::usage = "Massless momentum space one-loop integrals as defined in Pascual & Tarrach, Davydychev. Arguements are
integralDef[p,r,s,\!\(\*SubscriptBox[\(\[Mu]\), \(1\)]\),\!\(\*SubscriptBox[\(\[Mu]\), \(2\)]\),\!\(\*SubscriptBox[\(\[Mu]\), \(3\)]\)] = \[Integral]\!\(\*SuperscriptBox[\(d\), \(D\)]\)q \!\(\*FractionBox[\(\*SuperscriptBox[\(q\), SubscriptBox[\(\[Mu]\), \(1\)]] \*SuperscriptBox[\(q\), SubscriptBox[\(\[Mu]\), \(2\)]] \*SuperscriptBox[\(q\), SubscriptBox[\(\[Mu]\), \(3\)]]\), \(\*SuperscriptBox[\((\*SuperscriptBox[\(q\), \(2\)] + I\[Eta])\), \(r\)] \*SuperscriptBox[\((\*SuperscriptBox[\((q - p)\), \(2\)] + I\[Eta])\), \(s\)]\)]\).
Index arguments to function are optional; integralDef can handle scalar to rank-three tensor integrals."
vev::usage = "Vacuum expectation value envelope."
DDGG::usage = "Trigluon condensate term as a result of the second order expansion term of the 4D gluon condensate; result of the equation of motion."
gggGGG::usage = "Trigluon condensate. Implied \!\(\*SuperscriptBox[\(f\), \(abc\)]\)."
GG::usage = "Gluon condensate."
Begin["`Private`"]
...
tr/:tr[x__]/;EvenQ[Length[x]]&&Length[x]>2&&!MemberQ[x,\[Gamma][5]]&&MemberQ[x,Repeated[\[Gamma][_]]]&&FreeQ[x,lam[__]]&&FreeQ[x,C]&&FreeQ[x,\[Gamma]T[_]] && FreeQ[x,_Complex]&&FreeQ[x,vec[__]]&&FreeQ[x,m]&&FreeQ[x,M]:=Sum[((-1)^n)*g[Level[x[[1]],1][[1]],Level[x[[n]],1][[1]]]*tr[Drop[Drop[x,{1}],{n-1}]],{n,2,Length[x]}]
tr/:tr[A___**a_*B_**C___]/;MemberQ[B,\[Delta][_,_]]||MemberQ[B,vec[_,_]]||MemberQ[B,d]:=B*tr[A**a**C]
tr/:tr[\[Gamma][a_]]:=0
...
NonCommutativeMultiply/:NonCommutativeMultiply[A___,vec[a_,b_],B___]:=vec[a,b]A**B 
...
NonCommutativeMultiply/:NonCommutativeMultiply[A___,Times[x_,C_],D___]/;SameQ[x,gs]||SameQ[x,m]||SameQ[x,M]||MemberQ[{x},vec[_,_]]:=Times[x,NonCommutativeMultiply[A,C,D]]
NonCommutativeMultiply/:NonCommutativeMultiply[A___,vec[p_,\[Mu]_]\[Gamma][\[Mu]_],B___]:=vec[p,\[Mu]]*NonCommutativeMultiply[A,\[Gamma][\[Mu]],B]
...
tr/:tr[A___*p_*B___]/;FreeQ[p,\[Gamma][_]]&&FreeQ[p,lam[_]]&&FreeQ[p,\[Gamma][id]]&&FreeQ[p,C]&&FreeQ[p,\[Gamma]T[_]]:=p tr[A**B]
vec/:vec[a_*p_,\[Nu]_]/;a\[Element]Complexes:=a*vec[p,\[Nu]]
vec/:vec[p_,\[Mu]_]vec[q_,\[Mu]_]:=p.q
vec/:vec[p_,\[Mu]_]^2:=p.p
vec/:vec[p_+q_,\[Mu]_]:=vec[p,\[Mu]]+vec[q,\[Mu]]
(*Metric Identites*)
g/:g[\[Mu]_,\[Mu]_]:=d
...
vecConvert:={vec[p_,\[Mu]_]->p[\[Mu]], p.p->p^2, q.q->q^2,k.k->k^2}
vecUnConvert:={p_[\[Mu]_]->vec[p,\[Mu]], p^2->p.p, q^2->q.q,k^2->k.k}
...
integralDef[p_,r_,s_,\[Mu]_]:=(I \[Nu]^(2\[Epsilon]))/(4\[Pi])^2 (-p.p/(4\[Pi] \[Nu]^2))^\[Epsilon] (2\[Pi])^d/(p.p)^(r+s-2) vec[p,\[Mu]] (Gamma[3-r+\[Epsilon]]Gamma[2-s+\[Epsilon]]Gamma[r+s-2-\[Epsilon]])/(Gamma[r]Gamma[s]Gamma[5-r-s+2\[Epsilon]])
integralDef[p_,r_,s_,\[Mu]_,\[Nu]1_]:=(I \[Nu]^(2\[Epsilon]))/(4\[Pi])^2 (-p.p/(4\[Pi] \[Nu]^2))^\[Epsilon] (2\[Pi])^d/(p.p)^(r+s-2) (g[\[Mu],\[Nu]1]p.p (Gamma[3-r+\[Epsilon]]Gamma[3-s+\[Epsilon]]Gamma[r+s-3-\[Epsilon]])/(2 Gamma[r]Gamma[s]Gamma[6-r-s+2\[Epsilon]])+vec[p,\[Mu]]vec[p,\[Nu]1] (Gamma[4-r+\[Epsilon]]Gamma[2-s+\[Epsilon]]Gamma[r+s-2-\[Epsilon]])/(Gamma[r]Gamma[s]Gamma[6-r-s+2\[Epsilon]]))
(*eq. 17 Davydychev 1991;  took p \[Rule] -p to be consistent with Pascual & Tarrach conventions.*)
(**)
integralDef[p_,r_,s_,\[Mu]_,\[Nu]1_,\[Sigma]_]:=
((\[Pi]^(2+\[Epsilon]) I^(1-4-2\[Epsilon]))(p.p)^(2+\[Epsilon]-r-s))/(Gamma[r]Gamma[s]Gamma[4+2\[Epsilon]-r-s+3]) \[Nu]^(2\[Epsilon])/(\[Nu]^2)^\[Epsilon] (vec[p,\[Mu]]vec[p,\[Nu]1]vec[p,\[Sigma]] Gamma[2+\[Epsilon]-s]Gamma[3+2+\[Epsilon]-r]Gamma[r+s-2-\[Epsilon]]+1/2 p.p(g[\[Mu],\[Nu]1]vec[p,\[Sigma]]+g[\[Nu]1,\[Sigma]]vec[p,\[Mu]]+g[\[Sigma],\[Mu]]vec[p,\[Nu]1])Gamma[2+\[Epsilon]+1-s]Gamma[2+\[Epsilon]+3-1-r]Gamma[r+s-2-\[Epsilon]-1])
(*CONDENSATES*)
(*================*)
...
Format[vec[p_,\[Mu]_],StandardForm]:=Subscript[Style[p,Bold],\[Mu]]
