...
(*lowerbound\[Tau]::usage = "Finds the lower bound of \[Tau]."*)
(*Begin["`Private`"]*)
(*ratio[\[Tau]_,s0_,k_,jpc_,flavor_]:=laplace[\[Tau],s0,k+1,jpc,flavor]/laplace[\[Tau],s0,k,jpc,flavor]*)
(*upperbound\[Tau][k_,jpc_,flavor_]:=Module[{percent,cond1,cond2},*)
(*percent=1/3;*)
...
(*]*)
(*PC0[\[Tau]_,s0_,k_,jpc_,flavor_]:=laplace[\[Tau],s0,k,jpc,flavor]/Limit[laplace[\[Tau],s0,k,jpc,flavor],s0->Infinity]*)
(*lowerbound\[Tau][k_?NumericQ,s0_?NumericQ]:=\[Tau]/.FindRoot[{PC0[\[Tau],s0,k]==.1},{\[Tau],0.1}]*)
(*opts0[s0init_,lowerbound\[Tau]init_,k_,jpc_,flavor_]:=*)
...
(*tauInterval=Abs[upperbound\[Tau][k,jpc,flavor]-lowerbound\[Tau]initial]/20;avgM=Sum[ratio[\[Tau],s0,k,jpc,flavor]/Length[Table[\[Tau],{\[Tau],(lowerbound\[Tau]initial),(upperbound\[Tau][k,jpc,flavor]),tauInterval}]],{\[Tau],lowerbound\[Tau]initial,(upperbound\[Tau][k,jpc,flavor]),tauInterval}];*)
(*s0opt1=s0/.FindMinimum[{Sum[(ratio[\[Tau],s0,k,jpc,flavor]/avgM-1)^2,{\[Tau],lowerbound\[Tau]initial,(upperbound\[Tau][k,jpc,flavor]),tauInterval}]},{s0,s0initial},PrecisionGoal->0.001][[2]];*)
(*root1=FindRoot[Evaluate@PC0[\[Tau],s0opt1,k,jpc,flavor]==.1,{\[Tau],0.1}]//Chop;*)
...
(*s0opt2=s0/.FindMinimum[{Sum[(ratio[\[Tau],s0,k,jpc,flavor]/avgM-1)^2,{\[Tau],lowerbound\[Tau]1,(upperbound\[Tau][k,jpc,flavor]),tauInterval}]},{s0,s0initial},PrecisionGoal->0.001][[2]];*)
(*root2=FindRoot[Evaluate@PC0[\[Tau],s0opt2,k,jpc,flavor]==.1,{\[Tau],0.1}]//Chop;*)
